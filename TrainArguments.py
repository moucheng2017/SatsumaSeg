...
parser.add_argument('--repeat', type=int, help='number of times we repeat the experiment', default=1)
...
parser.add_argument('--unlabelled', type=int, help='ratio between unlabelled and labelled data in one batch', default=2)
parser.add_argument('--detach', type=bool, help='true when we cut the gradients in consistency regularisation', default=True)
parser.add_argument('--mu', type=float, help='prior Gaussian mean', default=0.5) # mu
parser.add_argument('--std', type=float, help='prior Gaussian std', default=0.1) # sigma
parser.add_argument('--alpha', type=float, help='weight on the unsupervised learning part', default=1.0)
parser.add_argument('--warmup', type=float, help='ratio between the iterations of warming up and the whole training iterations', default=0.1)
