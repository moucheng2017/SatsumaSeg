...
class RandomCroppingOrthogonalFast(object):
    def __init__(self,
                 cropping_d,
                 cropping_h,
                 cropping_w,
    ...
    def crop(self, *volumes):
        ...
        sample_position_d_d = np.random.randint(self.discarded_slices, d - self.volume_d[0])
        sample_position_d_h = np.random.randint(self.discarded_slices, h - self.volume_d[1])
        sample_position_d_w = np.random.randint(self.discarded_slices, w - self.volume_d[2])

        sample_position_h_d = np.random.randint(self.discarded_slices, d - self.volume_h[0])
        sample_position_h_h = np.random.randint(self.discarded_slices, h - self.volume_h[1])
        sample_position_h_w = np.random.randint(self.discarded_slices, w - self.volume_h[2])

        sample_position_w_d = np.random.randint(self.discarded_slices, d - self.volume_w[0])
        sample_position_w_h = np.random.randint(self.discarded_slices, h - self.volume_w[1])
        sample_position_w_w = np.random.randint(self.discarded_slices, w - self.volume_w[2])
...
class CT_Dataset_Orthogonal_Fast(torch.utils.data.Dataset):
    '''
    ...
    def __init__(self, imgs_folder, labels_folder, lung_folder, cropping_d, cropping_h, cropping_w, labelled):
        ...
        self.augmentation_cropping = RandomCroppingOrthogonalFast(cropping_d=cropping_d,
