...
def main():
    parser = argparse.ArgumentParser(description='Training options for semi supervised segmentation with bayesian pseudo labels.')
    ...
    for step in range(args.iterations):
        ...
        sup_loss = loss_d.get('supervised loss').get('seg loss') + loss_h.get('supervised loss').get('seg loss') + loss_w.get('supervised loss').get('seg loss')
        sup_loss = sup_loss.mean() / 3
        # print(sup_loss)

        pseudo_loss = loss_d.get('pseudo loss').get('pseudo loss') + loss_h.get('pseudo loss').get('pseudo loss') + loss_w.get('pseudo loss').get('pseudo loss')
        pseudo_loss = pseudo_loss.mean() / 3
        # print(pseudo_loss)

        kl_loss = loss_d.get('kl loss').get('kl loss') + loss_h.get('kl loss').get('kl loss') + loss_w.get('kl loss').get('kl loss')
        kl_loss = kl_loss.mean() / 3
        # print(kl_loss)
        ...
        print(
            ...
            'Train loss h: {:.4f}, '
            'Train loss w: {:.4f}, '
            'Train pseudo loss: {:.4f} '
            'Train kl loss: {:.4f}'.format(step + 1,
                                            args.iterations,
                                            optimizer.param_groups[0]["lr"],
                                            loss_d.get('supervised loss').get('seg loss'),
                                            loss_h.get('supervised loss').get('seg loss'),
                                            loss_w.get('supervised loss').get('seg loss'),
        ...
        writer.add_scalars('loss metrics', {'train loss d': loss_d.get('supervised loss').get('seg loss'),
                                            'train loss h': loss_h.get('supervised loss').get('seg loss'),
                                            'train loss w': loss_w.get('supervised loss').get('seg loss'),
                                            'train sup loss': sup_loss,
