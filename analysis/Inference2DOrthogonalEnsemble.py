...
def morphoclose(seg, data):
    ...
    for yi in range(y_im.shape[0]):
        y_im[yi,:,:] = binary_closing(output[yi,:,:].squeeze(),structure=se)
    for xi in range(x_im.shape[1]):
        x_im[:,xi,:] = binary_closing(output[:,xi,:].squeeze(),structure=se)

    # init closed image
    closed_im = np.logical_or(y_im, x_im)

    # new voxels by closing MUST be air voxels by threshold
    difference = np.logical_and(airvox, (closed_im.astype(int) - output.astype(int)).astype(bool))
    # add new confirmed airway voxels to union segmentation
    closed_im = np.logical_or(output, difference)
    return np.logical_or(output, closed_im)
